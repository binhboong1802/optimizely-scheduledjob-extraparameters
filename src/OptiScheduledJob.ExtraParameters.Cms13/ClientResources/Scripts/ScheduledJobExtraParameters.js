// Route of the protected module's controller, relative to the shell root.
const EXTRA_PARAMS_MODULE_ROUTE = 'OptiScheduledJob.ExtraParameters/ScheduledJobExtraParameters';

// CMS 13 moved admin/shell module URL segments off the EPiServer brand onto Optimizely:
// what used to be served under "/EPiServer/..." is now "/Optimizely/..." (e.g. the admin UI itself
// lives at "/Optimizely/Optimizely.Cms.UI.Admin/"). Rather than hardcode either spelling, derive the
// shell root from the page we are injected into - that also keeps working on sites which customise
// the UI path. The constant is only a fallback for an unexpected/empty path.
const EXTRA_PARAMS_SHELL_ROOT_FALLBACK = '/Optimizely/';

function getShellRoot() {
    const firstSegment = window.location.pathname.split('/').filter(Boolean)[0];
    return firstSegment ? '/' + firstSegment + '/' : EXTRA_PARAMS_SHELL_ROOT_FALLBACK;
}

const EXTRA_PARAMS_API_BASE = getShellRoot() + EXTRA_PARAMS_MODULE_ROUTE;

// The selectors below (#root, #sideBarNavigationRoot, .scheduled-job.settings, .scheduled-jobs-list,
// .nav-list__link a) and the "#/ScheduledJobs" hash route are internals of the admin SPA rather than
// a public contract, so re-check them against the admin UI when moving to a new CMS major.
